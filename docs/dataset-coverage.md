...
FreeTacMan and FoTa reached C3 in Step 04; Step 05 added C2 discovery for Tac2Pose and study-specific MIT GelSight resources plus C3 local loading for Touch100k. Step 06 added C2 discovery for TVL, the complete Meta TacBench umbrella, and OAHD plus C3 guarded/local loading for Meta's force/slip and pose releases, TacVerse, and CLAMP; Step 07 added release-specific C2 LMT records and C3 local loading for Penn HaTT XML and Mendeley pressure/IMU pairs. The generic CSV/NPY/NPZ reader does not raise a grade because it has no knowledge of a published dataset's manifests, semantics, terms, or citations.
...
| LMT Haptic Texture Database | Distinct 69-, 108-, and 184-material archives with release-dependent controlled/freehand signals, images, and ratings | Intended sequence mapping remains release-specific rather than inferring one native schema | Terms unclear; archive response unreliable; no checksums or verified manifest | C2 | Umbrella plus release records implemented in Step 07; no download or loader until terms and formats qualify |
| Penn HaTT | XML recordings, texture images, models, and rendering code | Sequence; validated vibration, force, position/speed, material, trial path, and per-channel timing | Attributed noncommercial Penn license; active archive unverified | C3 | Bounded local recorded-data XML adapter implemented in Step 07; models, images, and rendering stay outside the adapter |
| Mendeley Multimodal Tactile Texture | Pickle files by speed/texture plus reader notebook | Sequence; paired barometer and nine-axis IMU observations with artifact-derived 30/35/40 mm/s and texture/trial labels | CC BY 4.0; 3.83 GB ZIP; publisher SHA-256; pickle requires explicit trust | C3 | Confirmed resumable acquisition and guarded paired adapter implemented in Step 07; C4 awaits raw pressure/IMU schema fields |
