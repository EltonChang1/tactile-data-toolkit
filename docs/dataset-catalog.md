...
- **Packaging and size:** Tac2Pose's paper says real datasets are available from the project website, but the named project URL did not expose a verifiable file manifest during this audit. Total bytes, checksums, a stable archive schema, and active download links therefore remain unverified.
- **Rights and constraints:** The journal article is CC BY-NC 4.0, but that publication license is not evidence of the data license. The MIT study pages also lack a shared collection-level data license; integration must wait for dataset-specific terms and working files.
- **Proposed support:** Keep a precise catalog record now, then add a user-supplied image/pose/mesh adapter only after obtaining a sample manifest and confirming the data terms. Treat hardness, force/shear, and slip resources as separate study records rather than silently merging them under the Tac2Pose citation.
...
- **Packaging and size:** The official project distributes files through Google Drive rather than a versioned dataset hub. A complete hosted byte count, checksums, and immutable revision identifier are not published on the project page.
...
- **Proposed support:** Add a local-directory/JSONL adapter, paired-file validation, citation and noncommercial-use notices, and an optional downloader only if a stable official endpoint can preserve the required acknowledgment and terms.
